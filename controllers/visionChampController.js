const VisionChamp = require('../models/VisionChamp');

// @Method: Get
// @Route : api/vision-cham
// @Desc  : Get all game quiz
exports.getAll = async (req, res, next) => {
  try {
    const allGames = await VisionChamp.find();
...
// @Method: Get
// @Route : api/vision-champ/filtered
// @Desc  : Get all game quiz
exports.getAllFilter = async (req, res, next) => {
  try {
    const allGames = await VisionChamp.find({ status: true });
    return res.status(200).json(allGames);
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
...
// @Method: Get
// @Route : api/vision-cham/:id
// @Desc  : Get a game quiz with id
exports.find = async (req, res, next) => {
  try {
    const { id } = req.params;
    const game = await VisionChamp.findById(id);
...
exports.statusUpdate = async (req, res, next) => {
  try {
    ...
    const quiz = await VisionChamp.findById(id);
    if (!quiz) {
      return res.status(404).json({ message: 'Quiz not found' });
    }

    await VisionChamp.findByIdAndUpdate(id, {
      $set: { status: !!status },
    });
...
exports.create = async (req, res, next) => {
  try {
    ...
    const gameInstance = new VisionChamp({
      gameType,
      selectedGame,
      description,
...
exports.update = async (req, res, next) => {
  try {
    ...
    const parseOptions = JSON.parse(options);
    const { id } = req.params;
    const findGame = await VisionChamp.findById(id);
    if (!findGame) {
